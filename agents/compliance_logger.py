...
class ComplianceLoggerAgent:
    ...
    async def record(
        ...
        async def _write_files() -> ComplianceRecord:
            ...
            def _blocking_io():
                ...
                record = ComplianceRecord(
                    ...
                    amount=event.amount,
                    transaction_type=event.channel,
                )
            ...
            record = await asyncio.to_thread(_blocking_io)
            
            if self.db_service:
                await self.db_service.store_decision(record.to_dict())
            
            return record

        return await _write_files()
